...
(defvar agent-harnesses
  '(("claude"   "/opt/claude-code/bin/claude" "--dangerously-skip-permissions")
    ("codex"    "codex"    "--dangerously-bypass-approvals-and-sandbox")
    ("opencode" "opencode" nil))
  "Agent harnesses launchable in a ghostel terminal.
Each entry is (LABEL PROGRAM SKIP-SWITCH); SKIP-SWITCH is appended to
the command line when launched with skip-permissions enabled.")

(defun agent-harness--start (label)
  "Launch harness LABEL in a new ghostel terminal.
Appends the harness's skip-permissions switch when the menu's
\"Skip permissions\" toggle is on."
  (pcase-let* ((`(,name ,program ,switch) (assoc label agent-harnesses))
