...
(defun codex-dired ()
  ...
  (let* ((pod (string-trim
               (shell-command-to-string
                "kubectl get pods -n nakkaya -l app=codex -o jsonpath='{.items[0].metadata.name}'")))
         (tramp-kubernetes-namespace "nakkaya"))
    (dired (format "/kubernetes:basic-memory@%s:/app/data/" pod))))

(defun openclaw-dired ()
  (interactive)
  (let* ((pod (string-trim
               (shell-command-to-string
                "kubectl get pods -n nakkaya -l app=openclaw -o jsonpath='{.items[0].metadata.name}'")))
         (tramp-kubernetes-namespace "nakkaya"))
    (dired (format "/kubernetes:gateway@%s:/home/node/.openclaw/workspace/" pod))))
#+end_src
