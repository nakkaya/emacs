...
(eval-after-load 'paredit
  '(progn
     ...
     (define-key paredit-mode-map (kbd "C-q")       #'paredit-reindent-defun)
...
(let ((window-configuration (make-hash-table :test 'equal)))
  ...
  (defun jump-to-org-agenda ()
    (interactive)
    (when (not (string= (buffer-name (current-buffer)) "*Org Agenda(h)*"))
...
(when (file-exists-p na-agenda-folder)
  (run-with-idle-timer (* 10 60) t #'jump-to-org-agenda))
