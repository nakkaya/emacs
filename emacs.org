...
(eshell/alias "aider" "/opt/aider/bin/aider --dark-mode $*")
#+END_SRC
...
(defun mlflow-request (endpoint method data)
  (let* ((url-request-method method)
         (url-request-extra-headers
          '(("Content-Type" . "application/json")))
         (url-request-data (encode-coding-string data 'utf-8))
         (auth (concat mlflow-username ":" mlflow-password))
         (url-http-userid mlflow-username)
         (url-http-password mlflow-password)
         (url (concat mlflow-server-url "/api/2.0/mlflow/" endpoint)))
    (with-current-buffer
...
(defun mlflow-list-experiments ()
  (let* ((response (mlflow-request "experiments/search" "POST" 
                                   "{\"order_by\": [\"last_update_time DESC\"], \"max_results\": 1000}"))
         (json-object-type 'plist)
         (json-array-type 'list)
         (parsed-response (json-read-from-string response))
         (experiments (plist-get parsed-response :experiments)))
    (mapcar (lambda (exp)
              `(nil [,(plist-get exp :experiment_id) ,(plist-get exp :name)]))
            experiments)))

(defun mlflow-list-runs (experiment-id)
  (let* ((data (json-encode `(("experiment_ids" . [,experiment-id])
                              ("max_results" . 100)
                              ("order_by" . ["start_time DESC"]))))
         (response (mlflow-request "runs/search" "POST" data))
         (json-object-type 'plist)
         (json-array-type 'list)
         (parsed-response (json-read-from-string response))
         (runs (plist-get parsed-response :runs)))
    (mapcar 'mlflow-process-run runs)))

(defun mlflow-get-tag-value (m k)
  (let ((tag (cl-find k m :test #'string= :key (lambda (tag) (plist-get tag :key)))))
    (when tag
      (plist-get tag :value))))

(defun mlflow-process-run (run)
  (let* ((info (plist-get run :info))
         (data (plist-get run :data))
         (tags (plist-get data :tags))
         (metrics (plist-get data :metrics))
         (run-name (or (mlflow-get-tag-value tags "mlflow.runName") "No run name"))
         (start-time (/ (plist-get info :start_time) 1000.0))
         (end-time (/ (plist-get info :end_time) 1000.0))
         (duration (if (> end-time 0)
                       (round (/ (- end-time start-time) 3600.0) 2)
                     nil))
         (metric-value (mlflow-get-preferred-metric metrics))
         (metric-str (if metric-value (format "%.3f" metric-value) "N/A")))
    `(nil [,run-name
           ,(format-time-string "%Y-%m-%d %H:%M" (seconds-to-time start-time))
           ,(if duration (number-to-string duration) "N/A")
           ,metric-str
           ,(plist-get info :run_id)])))

(defun mlflow-get-preferred-metric (metrics)
  (let ((preferred-metrics '("max_val_dice" "max_val_accuracy" "test_dice" "test_f1")))
    (catch 'found
      (dolist (metric preferred-metrics)
        (let ((value (mlflow-get-tag-value metrics metric)))
          (when value
            (throw 'found value))))
      nil)))
...
(defun mlflow-show-run ()
  (let* ((entry (tabulated-list-get-entry))
    ...
    (when run-id
      (let* ((run-data (mlflow-get-run-data run-id))
             (output-buffer (get-buffer-create (format "*mlflow-run-%s*" run-id))))
        (with-current-buffer output-buffer
          (erase-buffer)
          (insert (mlflow-format-run-data run-data))
          (goto-char (point-min))
          (special-mode)
...
(defun mlflow-get-run-data (run-id)
  (let* ((response (mlflow-request (format "runs/get?run_id=%s" run-id) "GET" ""))
         (json-object-type 'plist)
         (json-array-type 'list)
         (parsed-response (json-read-from-string response)))
    parsed-response))

(defun mlflow-format-run-data (run-data)
  (let* ((run (plist-get run-data :run))
         (info (plist-get run :info))
         (data (plist-get run :data))
         (tags (plist-get data :tags))
         (metrics (plist-get data :metrics))
    ...
    (concat
     (format "Run ID: %s\n" (plist-get info :run_id))
     (format "Run Name: %s\n" (or (mlflow-get-tag-value tags "mlflow.runName") "No run name"))
     (format "Status: %s\n" (plist-get info :status))
     (format "Epoch: %s\n" (mlflow-get-metric-history-length run-id "accuracy"))
     "Metrics:\n"
     (mapconcat (lambda (metric)
                  (format "  %s: %.3f" (plist-get metric :key) (plist-get metric :value)))
                metrics "\n")
     "\nParameters:\n"
     (mapconcat (lambda (param)
                  (format "  %s: %s" (plist-get param :key) (plist-get param :value)))
                params "\n"))))

(defun mlflow-get-metric-history-length (run-id metric-key)
  (let* ((response (mlflow-request (format "metrics/get-history?run_id=%s&metric_key=%s" run-id metric-key) "GET" ""))
         (json-object-type 'plist)
         (json-array-type 'list)
         (parsed-response (json-read-from-string response))
         (metrics (plist-get parsed-response :metrics)))
    (number-to-string (length metrics))))

(defun mlflow-get-tag-value (tags key)
  (let ((tag (cl-find key tags :test #'string= :key (lambda (tag) (plist-get tag :key)))))
    (when tag
      (plist-get tag :value))))
