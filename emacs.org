...
(setq org-capture-templates
	...
	("i" "Identified TODO" entry
         (file+olp (lambda () (concat na-agenda-folder "notes.org")) "Identified" "Todo")
         "* TODO %?\n" :prepend t)
...
for run in runs:
    ...
    # Get val_dice or val_accuracy if available
    val_dice = round(run.data.metrics.get('val_dice'), 3)
    val_accuracy = round(run.data.metrics.get('val_accuracy'), 3)
    metric_value = val_dice if val_dice is not None else val_accuracy

    # Prepare the printed metric value
    metric_str = str(metric_value) if metric_value is not None else 'N/A'
    ...
    print(f\"{run_name},{start_time},{duration},{metric_str}\")
")
...
(define-key mlflow-experiments-mode-map (kbd "RET") 'mlflow-show-experiment-runs)
...
(define-derived-mode mlflow-experiment-runs-mode tabulated-list-mode "mlflow-experiment-runs"
  (setq tabulated-list-format [("Name" 20 nil) ("Created" 30 nil) ("Duration (h)" 30 nil) ("Metric" 10 nil)])
  (setq tabulated-list-padding 1)
  (add-hook 'tabulated-list-revert-hook 'mlflow-experiment-runs-mode-setup t)
  (tabulated-list-init-header))

(defun mlflow-experiment-runs-mode-setup ()
  (let ((experiment-id (tabulated-list-get-id)))
