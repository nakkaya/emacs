...
#+BEGIN_SRC emacs-lisp
  ...
  (setq na-agenda-files '("notes.org" "inbox.org" "bookmarks.org" "shopping.org"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun org-pull ()
    (interactive)
    (org-sync-cmd (concat "rclone copy -u -v dropbox:org/ " na-agenda-folder)))

  (defun org-push ()
    (interactive)
    (org-sync-cmd (concat "rclone --exclude .git/ sync -v " na-agenda-folder " dropbox:org/")))

  (defun os-run (program &rest args)
  ...
  (defun org-sync ()
    (interactive)
    (let* ((default-directory na-agenda-folder)
           (current-branch (magit-get-current-branch)))

      (org-save-all-org-buffers)

      (unless (zerop (car (os-run "git" "diff" "--exit-code")))
  ...
  (defun na-load-theme ()
    (progn
      ...
      (unless window-system
        ...
        (set-face-foreground 'org-level-1 "color-147")
        (set-face-foreground 'org-level-2 "color-147")
        (set-face-foreground 'org-level-3 "color-147")
        (set-face-foreground 'org-level-4 "color-147")
        (set-face-foreground 'org-level-5 "color-147")
        (set-face-foreground 'org-level-6 "color-147")
        (set-face-foreground 'org-level-7 "color-147")
        (set-face-foreground 'org-level-8 "color-147")

        (add-hook 'eshell-mode-hook
                  (lambda ()
                    (set-face-foreground 'eshell-ls-directory "color-24")))
