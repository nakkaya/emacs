...
** Skeletons
 ...
 #+BEGIN_SRC emacs-lisp
   (define-skeleton skel-org-babel-notebook-header
     ...
     nil
     "#+title: Notebook" \n
     "#+PROPERTY: header-args:jupyter-python :session /ssh:gpu:python :kernel python" \n
     "#+STARTUP: hidestars\n\n")
 #+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; nq exec remote file
  (defun eshell/rnq (host file)
    (let ((default-directory (concat "/ssh:" host ":~")))
      (eshell/echo
       (shell-command-to-string (concat "nq " file)))))
  ...
  (eshell/alias "rcp" "rsync -rLK $1 $2 --delete")
