...
read -p "Enable OpenCode (y/n) [n]? " with_opencode
with_opencode=${with_opencode:-n}
...
if [ "$with_opencode" == "y" ]; then
    opencode="--env OPENCODE_ENB=1"
else
    opencode=""
fi
...

{
    ...
    echo ""
    echo "    docker rm -f emacsd 2>/dev/null || true"
    echo ""
    echo "    $cmd"
    echo ""
