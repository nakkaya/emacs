...
** Input
   ...
   #+begin_src emacs-lisp 
     ...
     (cond 
      (na-os-is-darwin
       (progn 
         (define-key global-map [(control kp-delete)] 'na-next-term)
         (add-hook 'term-mode-hook
                   '(lambda ()
                      (define-key term-raw-map [(control kp-delete)] 'na-next-term)))))
      (na-os-is-linux
       (progn 
         (define-key global-map [(control delete)] 'na-next-term)
         (add-hook 'term-mode-hook
                   '(lambda ()
                      (define-key term-raw-map [(control delete)] 'na-next-term))))))
  ...
  #+begin_src emacs-lisp 
    (if window-system
      ...
      (progn 
        ...
        (set-face-background 'modeline "color-32")
...
** Dired
   ...
   #+begin_src emacs-lisp
     ...
     (setq sr-attributes-display-mask '(nil nil nil nil nil nil nil nil))
   ...
   #+begin_src emacs-lisp
     (defun lx-sunrise-display-external ()
       ...
       (let ((file (dired-get-filename))
         ...
         (start-process "" nil lx-sunrise-external-viewer file)))
...
*** Common
    ...
    #+begin_src emacs-lisp
      ...
      (setq lisp-programs 
            (list (list "clojure" (concat lein-path "lein trampoline run -m clojure.main/main"))
                  (list "nrepl" (concat lein-path "lein repl"))
