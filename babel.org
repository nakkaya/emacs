...
* Load Path
  #+begin_src emacs-lisp 
    ...
    (add-to-list 'load-path (concat dotfiles-dir "/ext/google-contacts/"))
  #+end_src
...
** Input
   ...
   #+begin_src emacs-lisp 
     (global-set-key [f1] 'na-gnus-switch)
...
*** Git Annex
    ...
    #+begin_src emacs-lisp
      (defun na-annex-async-add ()
        ...
        (let ((files (dired-get-marked-files nil current-prefix-arg))
          ...
          (setq dired-async-rsync-command "git annex add ")
          ...
          (tmtxt/dired-async dired-async-rsync-command "git-annex"
                             'tmtxt/dired-async-rsync-process-handler)))
    #+end_src
          ...
          (dolist (file files)
            (setq dired-async-rsync-command
                  (concat dired-async-rsync-command 
                          "git annex whereis " 
                          (file-name-nondirectory (shell-quote-argument file)) ";")))
          (async-shell-command dired-async-rsync-command "git annex whereis")
          (other-window 1)
    ...
    #+begin_src emacs-lisp
      ;; (setq kiler-repo-list (list "/home/nakkaya/Downloads/stg/"
      ;;                             "/home/nakkaya/Downloads/tmp1/stg/"
      ;;                             "/home/nakkaya/Downloads/tmp2/stg/"))
      
      (setq kiler-repo-list (list "/media/nakkaya/ozge/kiler/"
...
*** agenda
    ...
    #+begin_src emacs-lisp
      ...
      (setq org-mobile-files (list "~/Dropbox/org/gtd.org"))
...
** Gnus
   ...
   #+begin_src emacs-lisp
     ...
     (add-to-list 'mm-text-html-renderer-alist
                  '(vilistextum mm-inline-render-with-file nil
                    "/home/nakkaya/Dropbox/bash/wash-html.sh" file))
   ...
   #+begin_src emacs-lisp
     ...
     ;;
     ;; level 1: mail and local news groups are scanned.
     (defun gnus-demon-scan-news-and-update ()
       "Scan for new mail, updating the *Group* buffer."
       (message (concat (current-time-string) " Checking Mail..."))
       (gnus-demon-scan-mail-or-news-and-update 1))
     
     (defun na-init-demon-handlers ()
       (gnus-demon-add-handler 'gnus-demon-scan-news-and-update 1 1)
       (gnus-demon-add-handler 'gnus-group-save-newsrc 2 2))
   ...
   #+begin_src emacs-lisp
     ...
     (define-key gnus-summary-mode-map [(d)] 'na-gmail-move-trash)
     
     (defun na-gmail-archive ()
       (interactive)
       (gnus-summary-move-article nil "[Gmail]/All Mail"))
     
     (define-key gnus-summary-mode-map [(e)] 'na-gmail-archive)
     
     (defun na-mark-all-and-delete ()
       (interactive)
       (gnus-uu-mark-all)
       (na-gmail-move-trash))
     
     (define-key gnus-summary-mode-map (kbd "C-d") 'na-mark-all-and-delete)
   #+end_src
   ...
   #+begin_src emacs-lisp
     (defun na-gnus-switch ()
       "Switch to git buffer or run git-status"
       (interactive)
       (if (string= (buffer-name (current-buffer)) "*Group*")
           (jump-to-register 'y)
         (progn 
           ...
           (if (not (eq (get-buffer "*Group*") nil))
               (progn (delete-other-windows)
                      ...
                      (beginning-of-buffer))
             (progn (gnus)
                    (na-init-demon-handlers))))))
   #+end_src

*** Browsing URLs in Gnus Article
    ...
    #+begin_src emacs-lisp
      ...
      (define-key gnus-summary-mode-map [(v)] 'gnus-article-browse-urls)
      (define-key gnus-summary-mode-map [(f)] 'gnus-article-browse-url-first)
      (define-key gnus-summary-mode-map [(l)] 'gnus-article-browse-url-last)
    #+end_src
...
*** Google Contacts
    ...
    #+begin_src emacs-lisp
      (if (>= emacs-major-version 24)
          (progn (require 'oauth2)
                 (require 'google-contacts)
                 (require 'google-contacts-gnus)
                 (require 'google-contacts-message)
                 (add-hook 'message-mode-hook (function (lambda () (smart-tab-mode -1))))))
    #+end_src
