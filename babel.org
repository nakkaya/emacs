...
** Load/Install Melpa Packages
   ...
   #+begin_src emacs-lisp
     ...
     (setq package-list '(magit clojure-mode nrepl god-mode expand-region dash))
        ...
        (let* ((shell-out 
                (shell-command-to-string 
                 "for remote in $(git remote); do
                    printf \"$(df -k $(git config --get-regexp remote.$remote.url | awk '{print $2}') | tail -n +2 | awk '{print $4}') $remote \";
                  done;")))
          
          
          (->> (split-string shell-out " ")
            (-partition 2)
            (--sort (> (string-to-number (car it)) 
                       (string-to-number (car other))))
            (-map 'cdr)
            (-flatten))))
      ...
      (defun na-annex-replicate ()
        ...
        (let ((files (dired-get-marked-files nil current-prefix-arg))
          ...
          (dolist (file files)
            (setq annex-command
                  (concat annex-command
                          "for remote in "(mapconcat 'identity (na-annex-remotes-by-free-space) " ")"; do git annex copy " 
                          (file-name-nondirectory (shell-quote-argument file)) 
                          " --not --copies 2 --to $remote; done;")))
...
*** Mr
    ...
    #+begin_src emacs-lisp
      ...
      ;; (defvar na-mr-fast-push-timer)
      
      ;; (setq na-mr-fast-push-timer
      ;;       (run-with-idle-timer 600 t
      ;;                            (lambda ()
      ;;                              (na-mr-fast-push "~/"))))
      
      ;;(cancel-timer na-mr-fast-push-timer)
    #+end_src
...
*** agenda
    ...
    #+begin_src emacs-lisp
      (setq org-mobile-inbox-for-pull "~/annex/notes/from-mobile.org")
      (setq org-mobile-directory "~/annex/notes/mobile-org")
      (setq org-mobile-files (list "~/annex/notes/gtd.org"))
...
*** Capture
    ...
    #+begin_src emacs-lisp
      ...
      (setq org-capture-templates
            '(("l" "Link" entry (file+olp "~/annex/notes/gtd.org" "Read Later")
...
** mu4e
   ...
   #+begin_src emacs-lisp
     ...
     (setq mu4e-html2text-command "/home/nakkaya/.wash-html.sh")
   ...
   #+begin_src sh :tangle ~/.wash-html.sh
     vilistextum -l -r -c -s -m - -
     
     # for gnus
     # tmp=$(cat $1 | recode HTML_4.0)
     # echo $tmp > $1
     # vilistextum -l -r -c -s -m -u $1 -
   #+end_src
   ...
   #+BEGIN_SRC sh :tangle ~/.kill-offlineimap.sh
     #!/bin/bash
     
     ps aux | grep offlineimap | grep -v grep | awk "{ print \$2 }" | xargs kill -9
   #+END_SRC
   ...
   #+begin_src emacs-lisp
     ...
     (define-key mu4e-main-mode-map [(K)] 
       (defun na-kill-offlineimap ()
         ...
         (shell-command-to-string "~/.kill-offlineimap.sh")
         (sit-for 0.5)
         (offlineimap)
         (other-window 1)))
