...
** Misc
   ...
   This is a little function that is useful when editing several files
   that might for any reason change on disk (update from
   VersionControl). It reverts all buffers that are visiting a file. 
    ...
    #+begin_src emacs-lisp
      ...
      (setq lisp-programs 
            (list (list "clojure" clojure-command)
                  (list "lein1" "/Users/nakkaya/Dropbox/bash/lein1 repl")
